...
class PEPS(QuantumState):
    ...
    def apply(self, other):
        ...
        other = other.grid
        tn = np.empty_like(A)
        for (idx, a), b in zip(np.ndenumerate(A), other.flat):
    ...
    def contract_BMPS(self, mps_mult_mpo=None, **svdargs):
        ...
        return result.item() if result.size == 1 else result.reshape(*[2]*self.nrow*self.ncol).transpose(*[i+j*self.nrow for i, j in np.ndindex(*self.shape)])
